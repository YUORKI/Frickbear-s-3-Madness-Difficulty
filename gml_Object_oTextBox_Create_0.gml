...
function draw_everything()
{
    ...
    if ((((ForwardKey || TextPage == 0) && Delay[TextPage] == 0) || (Skip[TextPage] && TextDrawChar == TextLength[TextPage])) && !ExternalPause)
    {
        if (TextDrawChar == TextLength[TextPage])
        {
            if (TextPage < (TextPageNumber - 1))
            {
                ...
                if (is_callable(Function[TextPage]))
                    Function[TextPage]();
                
                if (array_length(CGPFunction) > 0 && TextPage < array_length(CGPFunction))
                {
                    switch (array_length(CGPFunction[TextPage]))
                    {
                        case 0:
                            break;
                        
                        case 1:
                            CGPFunction[TextPage][0]();
                            break;
                        
                        default:
                            CGPFunction[TextPage][0](CGPFunction[TextPage][1]);
                            break;
                    }
                }
    ...
    if (Speaker != "")
        NameboxWidth = (string_width(Speaker) + (XOffset * 2)) / 64;
    ...
    draw_sprite_ext(TextBoxSprite, 1, NameboxX, NameboxY, NameboxWidth, 1, 0, TextboxColor, 1);
    draw_text_color(round(CurrentTextboxX) + XOffset, round(CurrentNameboxY) + YOffset, Speaker, AltColor, AltColor, AltColor, AltColor, 1);
    OptionArrowOffset += ((TextOptionPosition - OptionArrowOffset) / 2);
    ...
    if (TextDrawChar == TextLength[TextPage] && TextPage == (TextPageNumber - 2))
    {
        ...
        if (TextOptionNumber > 0)
        {
            ...
            BGTarget += 64;
            OptionOffset += ((BGTarget - OptionOffset) / 5);
        }
        else
        {
            OptionOffset += ((-16 - OptionOffset) / 5);
        }
    ...
    draw_sprite_ext(TextBoxSprite, 0, round(CurrentTextboxX), round(CurrentTextboxY), 10, 2, 0, TextboxColor, 1);
    ...
    CurrentTextboxX += ((TextBoxX - CurrentTextboxX) / 5);
    CurrentTextboxY += ((TextBoxY - CurrentTextboxY) / 5);
    
    if (Speaker == "" || TextBoxY != 232)
        CurrentNameboxY += ((640 - CurrentNameboxY) / 5);
    else
        CurrentNameboxY += ((176 - CurrentNameboxY) / 5);
