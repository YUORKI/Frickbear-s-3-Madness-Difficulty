...
if (TilesLeft == 0 && !Dead)
{
    Dead = true;
    add_tokens(30);
    global.TokensPerMinigame[UnknownEnum.Value_0] += 30;
    
    if (global.HiScores[UnknownEnum.Value_0] > Timer)
        global.HiScores[UnknownEnum.Value_0] = Timer;
    
    if (global.HiScoresOverall[UnknownEnum.Value_0] > Timer)
        global.HiScoresOverall[UnknownEnum.Value_0] = Timer;
    
    if (floor(Timer) <= 15)
        get_trophy(UnknownEnum.Value_1);
}
