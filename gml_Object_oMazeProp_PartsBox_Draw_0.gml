...
vertex_set_point(VertexBuffer, fx(right, bottom), fy(right, bottom), fz(upper), 0, 0);
vertex_set_point(VertexBuffer, fx(leftmost, bottom), fy(leftmost, bottom), fz(24), 1, 1);

repeat (2)
{
    vertex_set_point(VertexBuffer, fx(left, bottom), fy(left, bottom), fz(upper), 1, 0.5);
    vertex_set_point(VertexBuffer, fx(leftmost, top), fy(leftmost, top), fz(24), 0, 1);
}

vertex_set_point(VertexBuffer, fx(left, top), fy(left, top), fz(upper), 0, 0.5);
vertex_set_point(VertexBuffer, fx(rightmost, bottom), fy(rightmost, bottom), fz(upper / 2), 1, 1);

repeat (2)
{
    vertex_set_point(VertexBuffer, fx(right, bottom), fy(right, bottom), fz(upper), 1, 0.5);
    vertex_set_point(VertexBuffer, fx(rightmost, top), fy(rightmost, top), fz(upper / 2), 0, 1);
}

vertex_set_point(VertexBuffer, fx(right, top), fy(right, top), fz(upper), 0, 0.5);
vertex_end(VertexBuffer);
vertex_submit(VertexBuffer, pr_trianglelist, sprite_get_texture(sMazeTex_PartsBox, 0));
